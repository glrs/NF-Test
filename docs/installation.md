...
To start using the NF-Test pipeline, there are three steps described below:
...
This pipeline itself needs no installation - NextFlow will automatically fetch it from GitHub if `glrs/NF-Test` is specified as the pipeline name.
...
```bash
wget https://github.com/glrs/NF-Test/archive/master.zip
unzip master.zip -d /my-pipelines/
cd /my_data/
nextflow run /my-pipelines/nf-test-master
```
